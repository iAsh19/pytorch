...
std::shared_ptr<compiler::GenericComputation> BuildNodeComputation(
    const torch::lazy::Node* node,
...
//       torch_lazy_tensors::compiler::getBackendRegistrar()->GetCompilationDevices(device,
//                                                                     devices);
...
//     auto computation_ptrs = torch_lazy_tensors::compiler::getBackendRegistrar()->Compile(
...
std::vector<compiler::DataPtr> OpByOpExecutor::Execute(
    c10::ArrayRef<torch::lazy::Value> roots, const std::string& device,
    c10::ArrayRef<std::string> devices) {
  // auto chained_exec_ops = BuildOps(roots, device, devices);
  throw std::runtime_error("TODO(whc) probably delete opbyopexecutor?");
  // return torch_lazy_tensors::compiler::getBackendRegistrar()->ExecuteChained(
